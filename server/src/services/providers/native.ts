...
export interface NativeExchange {
  /**
   * Код от SDK провайдера. Необязателен: у Яндекса на Android кода нет
   * вовсе — SDK отдаёт токен, и вместо кода приезжает подписанный JWT.
   */
  code?: string;
  /**
   * Подписанный провайдером JWT с личностью. Проверяется у нас, а не
   * принимается на слово: в этом вся разница между «приложение сказало,
   * кто человек» и «провайдер это подписал».
   */
  jwt?: string;
