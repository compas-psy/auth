...
describe("рабочий процесс выкладки", () => {
  ...
  it("живая проверка показывает подключённые способы входа", () => {
    // Ключи провайдера доходят до сервера через .env, и увидеть это
    // можно только на живом адресе. /v1/auth/methods отдаёт КОДЫ
    // способов, а не секреты, — смотреть безопасно.
    const steps = wf.jobs.deploy!.steps.map((st) => st.run ?? "").join("\n");
