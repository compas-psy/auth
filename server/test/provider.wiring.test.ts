...
describe("ключи Яндекса из окружения доходят до экрана", () => {
  ...
  it("на мобильном он тоже есть: нативный обмен кода подключён", async () => {
    // Проверка требовала ОБРАТНОГО, и это было верно ровно до того
    // дня, когда нативных адаптеров не существовало ни одного.
    //
    // Теперь перечень отвечает на вопрос «код какого провайдера
    // сервер готов обменять на личность». Есть ли у приложения
    // нативный SDK этого провайдера — знает только само приложение,
    // и оно рисует ПЕРЕСЕЧЕНИЕ: что умеет сервер и что умеет оно.
    const r = await app.inject({
