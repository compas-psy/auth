...
**Как поставить.** В npm пакет не опубликован, ставится из git:

```
npm i github:compas-psy/auth#main --workspace-root  # см. ниже
```

Точнее — пакет лежит в подкаталоге монорепозитория, поэтому:

```
npm i "git+https://github.com/compas-psy/auth.git#main:packages/id-client"
```

Если ваш npm не умеет подкаталоги — скопируйте `packages/id-client` к себе как vendored
зависимость и обновляйте по тегу. Сборка при установке идёт скриптом `prepare`; без него
`dist/` не собирался и `main` указывал в пустоту. Это была наша ошибка, нашёл её агент
ЗАПИСОК (`zapiski#4`, 10.09.2026), закрыто проверкой
`packages/id-client/test/installable.spec.ts`.
...
4. ~~Не выключать свой почтовый вход: он остаётся аварийным путём.~~
   **Отменено решением учредителя 10.09.2026.** На экране входа ЗАПИСОК остаётся ОДНА
   кнопка «Войти» — через СИМПАС. Уходят с экрана и Яндекс, и вход по почте.

   Что это значит по-честному: с этого момента наша недоступность — ваша недоступность
   для НОВЫХ входов. Уже вошедшие продолжают работать: свою сессию вы продлеваете своим
   refresh-токеном, к нам за этим ходить не нужно, и заметки на устройстве живут без сети
   по построению (И-4). Задевает вход на новом устройстве и после выхода.

   Решение принято с этим доводом на руках, а не вместо него.
